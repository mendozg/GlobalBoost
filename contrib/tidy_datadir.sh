#!/bin/bash
# Copyright (c) 2013 The GlobalBoost Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete GlobalBoost database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no GlobalBoost datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old GlobalBoost datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected GlobalBoost 0.7 datadir."
    ;;
  3)
    echo "Detected GlobalBoost pre-0.8 datadir."
    ;;
  4)
    echo "Detected GlobalBoost 0.8 datadir."
    ;;
esac
