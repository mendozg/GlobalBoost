...
The RPM spec file provided here is for GlobalBoost-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://globalboost.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[GlobalBoost git repository](https://github.com/globalboost/globalboost) but are not part of
...
Currently only `Source10` is used, for the example `globalboost.conf` file.
...
in the globalboost git repository. At present only an SVG version of the GlobalBoost
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the globalboost
...
needed when 0.12.1 is released, a proper fix is already in the GlobalBoost git
master branch.
...
idea to build GlobalBoost Core against a static version of that library compiled
...
The `globalboost.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `globalboost-server` package. At this point, most RPM based
...
The `globalboost-server` package should have SELinux support. How to properly do
...
GlobalBoost-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/globalboost/). He has been
packaging GlobalBoost for Fedora at least since 2012.
...
4. I renamed the `globalboost` package that contains the Qt GUI to `globalboost-core` as
that appears to be how the general population refers to it, in contrast to
`globalboost-xt` or `globalboost-classic`. I wanted to make sure the general population
