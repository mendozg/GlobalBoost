...
make check
pushd src
srcdir=. test/globalboost-util-test.py
popd
...
%{_sbindir}/semanage port -a -t globalboost_port_t -p tcp 8225
%{_sbindir}/semanage port -a -t globalboost_port_t -p tcp 8226
%{_sbindir}/semanage port -a -t globalboost_port_t -p tcp 18225
%{_sbindir}/semanage port -a -t globalboost_port_t -p tcp 18226
...
if [ $1 -eq 0 ]; then
	if [ `%{_sbindir}/sestatus |grep -c "disabled"` -eq 0 ]; then
	%{_sbindir}/semanage port -d -p tcp 8225
	%{_sbindir}/semanage port -d -p tcp 8226
	%{_sbindir}/semanage port -d -p tcp 18225
	%{_sbindir}/semanage port -d -p tcp 18226
	for selinuxvariant in %{selinux_variants}; do
