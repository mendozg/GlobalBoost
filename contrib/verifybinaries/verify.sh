...
###   globalboostcore.org and globalboost.org and compares them.
...
WORKINGDIR="/tmp/globalboost_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://globalboostcore.org"
HOST2="https://globalboost.org"
BASEDIR="/bin/"
VERSIONPREFIX="globalboost-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'globalboost-',
...
if [ $? -ne 0 ]; then
   echo "globalboost.org failed to provide signature file, but globalboostcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "globalboost.org and globalboostcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the GlobalBoost Core binary release signing key installed?"
   fi
